...
if ($_POST) {
    ...
    if ($difference == 0) {
        ...
        $query1 = " UPDATE regle_client SET  id_client=:id_client, id_card_bank=:id_card_bank, 
                    date_transaction=:date_transaction, id_chantier=:id_chantier, date_echeance=:date_echeance, montant=:montant, avance=:avance, reste=:reste, cheque=:cheque, id_mode_paiement=:id_mode_paiement WHERE id_regle_client = '$id' ";
        ...
        if ($sql1) {
            ?>
            <script>
                alert('Règlement client a été bien mis à jour.');
            ...
            ?>
            <script>
                alert('Règlement client n\'a pas été mis à jour.');
                window.location.href = '<?=$reglement_client['option2_link']?>?witness=-1';
            </script>
            <?php
        ...
        <script>
            ...
            window.location.href = '<?=$reglement_client['option2_link']?>?witnes=-3';
        </script>
        <?php
    }
