...
if ($_POST) {
    $id = $_POST['id_marche'];
    $id_rec_marche = $_POST['id_rec_marche'];
    $date_pj = date('y-m-d');
...
//            echo '-> : '.$file_dest.'<br/>';
        ...
        if (in_array($file_extension, $autorized_extensions)) {
            if ((move_uploaded_file($file_content, $file_dest) && ($_FILES['fichier']['error'] == 0))) {
                ...
                $stmp = "INSERT INTO pj_reception (id_marche,id_rec_marche, nom_pj, lien,date_pj)
                                                VALUES('$id','$id_rec_marche','$file_name', '$file_dest','$date_pj')";
                $sql = $db->query($stmp);
